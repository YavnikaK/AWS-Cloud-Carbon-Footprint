...
export default class CostAndUsageReports {
  ...
  getEstimatesFromInputData(
    ...
    console.log("inp data len in get estimates: ", inputData.length);
    ...
    inputData.map((inputDataRow: LookupTableInput) => {
      const usageRowsHeader = {
        Data: [
          ...
          // { VarCharValue: 'usageTypeClass' }
        ],
      }
      const usageRowData = [
        { VarCharValue: inputDataRow.timestamp },
        { VarCharValue: '' },
        ...
        { VarCharValue: '1' },
        { VarCharValue: inputDataRow.usageAmount } // instead of '1'
        // { VarCharValue: '' }
      ]
      const costAndUsageReportRow = new CostAndUsageReportsRow(
        usageRowsHeader,
      ...
      if (footprintEstimate) {
        // console.log(inputDataRow.timestamp);
        result.push({
          usageAmount: inputDataRow.usageAmount,
          timestamp: inputDataRow.timestamp,
          ...
          // usageAmount: inputDataRow.usageUnit,
          usageTypeClass: footprintEstimate.usageTypeClass
        })
      }
    })

    console.log("len of results before the unknown: ", result.length);
    ...
    console.log("len of results after the unknown: ", result.length);
