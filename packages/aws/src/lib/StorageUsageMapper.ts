...
export function getEstimatesFromCostExplorer(
  ...
  const footprintEstimates = [
    ...ssdEstimator.estimate("ssd-storage", ssdUsage, region, emissionsFactors, constants),
    ...hddEstimator.estimate("hdd-storage", hddUsage, region, emissionsFactors, constants),
  ]
  ...
  return Object.values(
    footprintEstimates.reduce(
      (acc: { [key: string]: MutableFootprintEstimate }, estimate) => {
        ...
        acc[estimate.timestamp.toISOString()].usageTypeClass = estimate.usageTypeClass
        return acc
      },
      {},
    ),
